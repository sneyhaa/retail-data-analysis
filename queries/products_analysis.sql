...
-- 8. How Product Preferences Vary by Income Levels
-- This query investigates the product preferences of different income level customers, particularly for 'Loafers'.
...
-- This query finds the products that generate the most revenue among each income bracket.eg: High
...
WITH ranked_gender_favorites AS (
    SELECT 
       	products,
        COUNT(DISTINCT customer_id) AS No_of_Customers,
        gender
...
ranked_by_gender AS (
    SELECT *,
        DENSE_RANK() OVER (PARTITION BY gender ORDER BY No_of_Customers DESC) AS rank
    FROM ranked_gender_favorites
)
SELECT *
